...
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBlZmxnZmVpZXF0a2xjcGtoc3p6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3MzEyMDEzNzksImV4cCI6MjA0Njc3NzM3OX0.OlEbttWuDvHHy9svUAr2quK4IrmRgkGUI0i8Z9LHfrU';
...
const productConfigs = [
  {
    ...
    retail_price: 24.99,
    variants: {
      8024: { size: "S", price: 14.50 },
      8025: { size: "M", price: 14.50 },
      8026: { size: "L", price: 14.50 },
      8027: { size: "XL", price: 14.50 },
      8028: { size: "2XL", price: 16.00 }
    },
    body: {
...
const getSyncedProducts = async (req, res) => {
  try {
    const response = await printfulClient.get(`/store/products?store_id=${PRINTFUL_STORE_ID}`);
    
    // Add pricing information from our configurations
    const products = response.data.result.map(product => {
      const config = productConfigs.find(cfg => cfg.product_id === product.sync_product.product_id);
      if (config) {
        product.pricing = {
...
const getMockupUrl = async (taskKey) => {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      const response = await printfulClient.get(`/mockup-generator/task?task_key=${taskKey}&store_id=${PRINTFUL_STORE_ID}`);
      ...
      console.error(`Error checking mockup status (attempt ${attempt + 1}):`, error.message);
  ...
  console.error(`Mockup generation timed out after ${maxAttempts} attempts for task: ${taskKey}`);
  return null;
};
